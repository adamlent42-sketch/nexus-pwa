...
export function PORecapForm({ open, onClose, po }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* What happens next key */}
      <details className="group mt-2">
        <summary className="flex items-center gap-1.5 cursor-pointer select-none list-none text-[12px] text-ink-tertiary hover:text-ink-secondary transition-colors py-1">
          <span className="w-3.5 h-3.5 rounded-full border border-line inline-flex items-center justify-center text-[9px] font-bold shrink-0">?</span>
          <span>What happens when you submit</span>
          <ChevronDown className="w-3 h-3 group-open:hidden" />
          <ChevronUp className="w-3 h-3 hidden group-open:block" />
        </summary>
        <div className="mt-2 rounded-lg border border-line bg-surface-subtle text-[11px] overflow-hidden">
          <div className="px-3 py-2.5 border-b border-line">
            <p className="font-semibold text-ink-secondary mb-1.5">Attended — outcome → lifecycle stage → what the system does</p>
            <div className="space-y-1 text-ink-secondary leading-snug">
              <div><span className="font-medium">Plan to Enroll + start date set</span> → <em>Pending Start</em> → welcome email goes out 7 days before first class</div>
              <div><span className="font-medium">Plan to Enroll, no start date yet</span> → <em>PO Attended – Plan to Enroll</em> → system follows up every few days until a date is locked in</div>
              <div><span className="font-medium">Undecided</span> → <em>PO Attended – Undecided</em> → low-pressure follow-ups over the next 30 days</div>
              <div><span className="font-medium">Not Interested</span> → <em>No Interest</em> → no further outreach sent</div>
            </div>
          </div>
          <div className="px-3 py-2.5 border-b border-line">
            <p className="font-semibold text-ink-secondary mb-1.5">Non-attended statuses</p>
            <div className="space-y-1 text-ink-secondary leading-snug">
              <div><span className="font-medium">Not Attended (no-show)</span> → <em>PO No-Show</em> → reschedule offer sent (3 follow-ups over 45 days)</div>
              <div><span className="font-medium">Family Cancelled</span> → <em>PO Cancelled</em> → gentle winback sequence (4 touches over 30 days)</div>
              <div><span className="font-medium">Rescheduled / Instructor Cancelled</span> → student stays where they are; create a new PO for the rescheduled date</div>
            </div>
          </div>
          <div className="px-3 py-2 bg-tint-notes-bg text-tint-notes-fg">
            Recap applies immediately — lifecycle updates on save. A follow-up email draft is created in Gmail for Adam to review before it goes out.
          </div>
        </div>
      </details>
    </Modal>
