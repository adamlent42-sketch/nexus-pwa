...
// GET /api/admin/po-recaps/history?q=name&offset=0
// Returns all past POs (any recap status) sorted by date desc.
// Used by the History tab to look up any PO ever.
export async function GET(req: NextRequest) {
  try {
    ...
    const nameFilter = q.length >= 2
      ? `SEARCH(LOWER("${q.replace(/"/g, "")}"), LOWER({Student Display}))`
      : "";

    const dateFilter = `AND({PO Date}, IS_BEFORE({PO Date}, DATEADD('${today}', 1, 'days')), NOT({Outcome} = 'Enrolled'))`;

    const filterByFormula = nameFilter
    ...
    // Last contact date per family — same logic as main recaps route
    const lastContactByFamily = new Map<string, string>();
    try {
      const commRecs = await airtable()(TABLE.Communications)
        .select({
          filterByFormula: `AND({Date}, IS_AFTER({Date}, DATEADD('${today}', -365, 'days')))`,
