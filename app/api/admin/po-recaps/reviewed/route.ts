...
export async function GET(req: NextRequest) {
  try {
    ...
    const records = await airtable()(TABLE.POs)
      .select({
        filterByFormula: `AND(
          {Recap Status} = 'Reviewed',
          IS_AFTER({PO Date}, DATEADD('${today}', -90, 'days')),
          NOT({Outcome} = 'Enrolled'),
          NOT({Outcome} = 'Not Interested')
        )`,
    ...
    // Last contact date per family (same logic as main route)
    ...
    try {
      ...
      for (const rec of commRecs) {
        ...
        for (const fid of families) {
          if (!lastContactByFamily.has(fid) && date) lastContactByFamily.set(fid, date);
        }
      }
    } catch { /* non-fatal */ }
